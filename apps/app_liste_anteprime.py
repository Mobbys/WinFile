...
# Import necessario per la funzionalità Drag & Drop
...
class FileScannerApp(ctk.CTkFrame):
    ...
    def handle_drop(self, event):
        """Questo metodo viene ora chiamato direttamente da winfile.py"""
        ...
        if paths:
            ...
            self.status_text.set("Nessun file o cartella valida trovata.")
    ...
    def print_table(self, selection_mode=False):
        ...
        if not item_ids:
            messagebox.showinfo("Informazione", "Nessuna riga da stampare."); return
    ...
    def export_to_csv(self, selection_mode=False):
        ...
        if not results_to_export:
            messagebox.showinfo("Informazione", "Nessun dato da esportare."); return

        file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("File CSV", "*.csv")], title="Salva lista come CSV")
        ...
        try:
            ...
            traceback.print_exc(); self.status_text.set("Errore durante la creazione del CSV.")
    ...
    def export_to_html(self, selection_mode=False):
        ...
        self.status_text.set("Creazione anteprima HTML in corso...")
        self.update_idletasks()
        
        try:
            html_content = self._generate_html_content(results_to_export)
            ...
            traceback.print_exc(); self.status_text.set("Errore durante la creazione dell'anteprima HTML.")
    ...
    def export_to_pdf(self, selection_mode=False):
        ...
        file_path = filedialog.asksaveasfilename(defaultextension=".pdf", filetypes=[("File PDF", "*.pdf")], title="Salva report PDF")
        if not file_path: return
